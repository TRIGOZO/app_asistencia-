<script setup>
  ...
  const { openModal, Toast, Swal, formatoFecha, meses } = useHelper();
    ...
    const {
        cargarReporteHorasExtras, registrosHorariosExtras
    } = useMarcacion();
    ...
    const jsonFields = ref({
        ...
        "Minutis Extra": "diferencia_salida",
    })
    onMounted(()=>{
    ...
    const buscar=async()=>{
        await cargarReporteHorasExtras(dato.value)
        // let marcaciones = registrosHorariosExtras.value
        ...
        // registrosHorariosExtras.value.forEach(item=>{
        //     item.descuento = (item.constante_descuento*item.minutos).toFixed(2)
        // });
    }
            ...
            <div class="card-body">
                <div class="row mb-4">
                    ...
                    <div class="col-md-2">
                        ...
                        <JsonExcel class="btn btn-success" :fields="jsonFields" :data="registrosHorariosExtras">
                    ...
                    <div class="col-md-12 mb-1">
                        <div class="table-responsive">         
                            <table class="table table-bordered table-hover table-sm table-striped">
                                ...
                                <tbody>
                                    <tr v-for="(registro,index) in registrosHorariosExtras" :key="registro.id">
                                        <td class="text-center">{{ index+1 }}</td>
                                        <td>{{ registro.numero_dni }}</td>
                                        <td>{{ registro.apenom }}</td>
                                        <td>{{ registro.diferencia_salida }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
